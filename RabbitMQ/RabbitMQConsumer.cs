...
public sealed class RabbitMQConsumer : IEventConsumer, IAsyncDisposable
{
    private readonly ConnectionFactory _factory;
    private IConnection? _connection;
    private IChannel? _channel;
    ...
    public async Task SubscribeAsync(string queue, string exchange, CancellationToken ct)
    {
        Console.WriteLine($"[Consumer] Connecting to RabbitMQ host={_factory.HostName}, user={_factory.UserName}");
        _connection = await _factory.CreateConnectionAsync(ct);
        _channel = await _connection.CreateChannelAsync();
        Console.WriteLine("[Consumer] Connection and channel created");

        await _channel.ExchangeDeclareAsync(exchange, ExchangeType.Fanout, durable: true, autoDelete: false);
        Console.WriteLine($"[Consumer] Declared exchange '{exchange}' (fanout)");

        await _channel.QueueDeclareAsync(queue, durable: true, exclusive: false, autoDelete: false);
        Console.WriteLine($"[Consumer] Declared queue '{queue}'");

        await _channel.QueueBindAsync(queue, exchange, "");
        Console.WriteLine($"[Consumer] Bound queue '{queue}' to exchange '{exchange}' with routingKey=''");
        ...
        CancellationToken ct)
    {
        ...
        Console.WriteLine($"[Consumer] Starting BasicConsume on queue='{_queue}'");

        var consumer = new AsyncEventingBasicConsumer(_channel);

        consumer.ReceivedAsync += async (_, ea) =>
        {
            Console.WriteLine($"[Consumer] Message received on queue='{_queue}', tag={ea.DeliveryTag}, size={ea.Body.Length}");
            var ok = await handler(ea.Body, ct);

            if (ok)
            {
                Console.WriteLine($"[Consumer] Acking tag={ea.DeliveryTag}");
            ...
            {
                Console.WriteLine($"[Consumer] Nacking tag={ea.DeliveryTag}");
        ...
        Console.WriteLine($"[Consumer] BasicConsumeAsync registered, waiting for messages...");
        await Task.Delay(Timeout.Infinite, ct);
    }
